(function ($) {
    $(window).on('load', function () {
        ...
        const intervalId = setInterval(() => {
            ...
            if (checkoutForm || cartFlowDiv) {
                symbol = lknWcipRecurringDonationVariables.symbol
                clearInterval(intervalId);
                
                if(checkoutForm){
                    recurringDonatePaymentHTML = `
                        ...
                        <div class="wcPaymentInvoiceInner">
                            <div class="wcPaymentInvoiceCheckboxWrapper">
                                <div class="wc-block-components-checkbox wc-block-checkout__use-address-for-billing">
                                    <label for="wcPaymentInvoiceContainerCheckboxRecurringDonate" class="wcPaymentInvoiceCheckboxLabel">
                                        <input id="wcPaymentInvoiceContainerCheckboxRecurringDonate" class="wc-block-components-checkbox__input" type="checkbox" aria-invalid="false">
                    ...
                    <div class="wcPaymentInvoiceContainer">
                        <div class="wcPaymentInvoiceInner">
                            <div class="woocommerce-shipping-fields">
                                <label for="wcPaymentInvoiceContainerCheckboxRecurringDonate" class="woocommerce-form__label woocommerce-form__label-for-checkbox checkbox wcPaymentInvoiceCheckboxLabelCartFlow">
                                    <input id="wcPaymentInvoiceContainerCheckboxRecurringDonate"
                ...
                tempDiv.innerHTML = recurringDonatePaymentHTML;
                const recurringDonatePaymentElement = tempDiv.firstElementChild;
                ...
                if(checkoutForm){
                    ...
                    if(methodsElement){
                        checkoutForm.insertBefore(recurringDonatePaymentElement, methodsElement)
                    }else{
                        checkoutForm.insertBefore(recurringDonatePaymentElement, fourthElement)
                    }
                }else{
                    ...
                    if (parentElement && parentElement.parentNode) {
                        parentElement.parentNode.insertBefore(recurringDonatePaymentElement, parentElement);
                    }
                }
    ...
    function modifyCheckoutData(data) {
        try {
            const checkboxRecurringDonate = document.getElementById('wcPaymentInvoiceContainerCheckboxRecurringDonate');
            const isRecurringDonation = checkboxRecurringDonate && checkboxRecurringDonate.checked;
            ...
            // Adiciona a informação da doação recorrente
            parsedData.recurring_donation = isRecurringDonation ? '1' : '0';
    ...
    function addHiddenFieldToForm() {
        ...
        if (checkoutForm && !document.getElementById('recurring_donation_hidden')) {
            const hiddenField = document.createElement('input');
            hiddenField.type = 'hidden';
            hiddenField.id = 'recurring_donation_hidden';
            hiddenField.name = 'recurring_donation';
            hiddenField.value = '0';
            checkoutForm.appendChild(hiddenField);
            
            // Atualiza o valor do campo hidden quando a checkbox muda
            const checkboxRecurringDonate = document.getElementById('wcPaymentInvoiceContainerCheckboxRecurringDonate');
            if (checkboxRecurringDonate) {
                checkboxRecurringDonate.addEventListener('change', function() {
        ...
        const observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                if (mutation.type === 'childList') {
                    const checkoutForm = document.querySelector('.wc-block-components-form.wc-block-checkout__form');
                    if (checkoutForm && !checkoutForm.hasAttribute('data-recurring-listener')) {
                        checkoutForm.setAttribute('data-recurring-listener', 'true');
                        
                        // Adiciona listener para quando o formulário é submetido
                        checkoutForm.addEventListener('submit', function(e) {
                            const checkbox = document.getElementById('wcPaymentInvoiceContainerCheckboxRecurringDonate');
                            if (checkbox && checkbox.checked) {
                                // Adiciona dados aos componentes do WooCommerce Blocks
                                if (window.wc && window.wc.wcBlocksData) {
                                    window.wc.wcBlocksData.recurring_donation = '1';
                                }
                            }
                        });
                    }
