(function ($) {
  $(window).load(function () {
    ...
    if (mainForm && fistH1 && submitP && tables) {
      ...
      document.querySelectorAll('.form-table > tbody > tr').forEach(tr => {
        ...
        if (forminp && label) {
          ...
          // Verificar se existem múltiplos fieldsets
          const fieldsets = forminp.querySelectorAll('fieldset')
          
          if (fieldsets.length > 1) {
            // Processar múltiplos fieldsets separadamente
            fieldsets.forEach((currentFieldset, index) => {
              const currentLegend = currentFieldset.querySelector('legend')
              // Usar o título do data-attribute ou legend
              let sectionTitle = currentFieldset.getAttribute('data-section-title') || 
                                (currentLegend ? currentLegend.textContent.trim() : titleText)
              
              if (currentLegend && currentFieldset) {
                processFieldset(currentFieldset, currentLegend, sectionTitle)
              }
            })
          } else if (legend && fieldset) {
            // Processar fieldset único (lógica original)
            processFieldset(fieldset, legend, titleText)
          } else {
            // Processar campo sem fieldset (lógica original para campos sem fieldset)
            processNonFieldsetField(forminp, titleText)
          }
        }
      })
      
      // Função para processar um fieldset individual
      function processFieldset(fieldset, legend, titleText) {
      ...
      // Função para processar campo sem fieldset
      function processNonFieldsetField(forminp, titleText) {
      ...
      // Aplica estilos para todos os elementos forminp
      document.querySelectorAll('.form-table > tbody > tr').forEach(tr => {
        const forminp = tr.querySelector('.forminp')
        if (forminp) {
    ...
    // Funcionalidade de prévia dos ícones dos métodos de pagamento
    $(document).on('change', '.lkn-icon-select', function() {
      const selectedIcon = $(this).val();
      const gatewayId = $(this).data('gateway');
      const previewDiv = $('.lkn-icon-preview[data-gateway="' + gatewayId + '"]');
      const iconUrl = wcInvoicePaymentData.pluginUrl + 'Public/images/' + selectedIcon + '.svg';
