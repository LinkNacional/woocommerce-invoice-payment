...
<div class="wrap">
    ...
    <nav class="nav-tab-wrapper woo-nav-tab-wrapper">
        <a
            ...
            class="nav-tab <?php echo is_active_tab('Invoices', $current_tab); ?>"
        >
            <?php esc_attr_e('Invoices', 'wc-invoice-payment') ?>
        </a>
        <a
            href="admin.php?page=settings&settings=Subscriptions"
            class="nav-tab <?php echo is_active_tab('Subscriptions', $current_tab); ?>"
        >
                            ...
                            <div class="input-column-wrap">
                                <label for="lkn_wcip_after_save_button_email_check">
                                   ...
                                   <?php if ($email_verify) {
                                    ...
                                    <div class="tooltip">
                                        <span
                                            class="tootip w-5 h-5 flex items-center justify-center text-white rounded-full cursor-pointer"
                                        >?</span>
...
<?php
    ...
    public function render_edit_invoice_page(): void {
        ...
        if (isset($countries['BR'])) {
            $brazil = $countries['BR'];
            unset($countries['BR']);
            $countries = ['BR' => $brazil] + $countries; // Coloca 'BR' no início
        }
                        ...
                        <select name="lkn_wcip_country" id="lkn_wcip_country_input" class="regular-text">
                            <?php
                                foreach ($countries as $code => $currency_name) {
                                    ...
                                    echo '<option value="' . esc_attr($code) . '" ' . $selected . '>' . esc_attr($currency_name) . '</option>';
                                }
                            ?>
                        </select>
                    </div>
