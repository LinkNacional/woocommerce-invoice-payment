<div class="show_if_donation dokan_donation_settings">
	<p>
		<label for="_donation_type"><?php esc_html_e('Donation type', 'wc-invoice-payment'); ?> <i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e('Select the donation type.', 'wc-invoice-payment'); ?>"></i></label>
		<select class="dokan-form-control" id="_donation_type" name="_donation_type">
			<option value="fixed" <?php selected($_donation_type, 'fixed'); ?>><?php esc_html_e('Fixed Amount (Donate item with fixed value item)', 'wc-invoice-payment'); ?></option>
			<option value="variable" <?php selected($_donation_type, 'variable'); ?>><?php esc_html_e('Variable Amount (Receive monetary donations)', 'wc-invoice-payment'); ?></option>
			<option value="free" <?php selected($_donation_type, 'free'); ?>><?php esc_html_e('Free (Donate an item)', 'wc-invoice-payment'); ?></option>
		</select>
	</p>
	...
	<p class="show_if_donation_variable">
		<label for="_donation_button_values"><?php esc_html_e('Preset button values', 'wc-invoice-payment'); ?> <i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e('Enter the preset button values separated by comma. E.g.: 10, 20, 25', 'wc-invoice-payment'); ?>"></i></label>
	...
	<p class="show_if_donation_variable" id="_donation_hide_custom_amount_field">
		<label>
			<input type="checkbox" class="_donation_hide_custom_amount" name="_donation_hide_custom_amount" id="_donation_hide_custom_amount" value="yes" <?php checked($_donation_hide_custom_amount, 'yes'); ?>> <?php esc_html_e('Hide custom amount field', 'wc-invoice-payment'); ?> <i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e('Check this option to hide the custom amount field and show only the preset buttons.', 'wc-invoice-payment'); ?>"></i>
		</label>
	</p>
	<p class="show_if_donation_free">
		<label for="_donation_free_text"><?php esc_html_e('Text', 'wc-invoice-payment'); ?> <i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e('Text to be displayed for free donation.', 'wc-invoice-payment'); ?>"></i></label>
		<input type="text" class="short dokan-form-control" name="_donation_free_text" id="_donation_free_text" value="<?php echo esc_attr($_donation_free_text); ?>" placeholder="<?php esc_attr_e('Free', 'wc-invoice-payment'); ?>">
	</p>
	
	<!-- === CAMPOS DE META DE DOAÇÃO === -->
	<div class="donation-goal-section dokan-section">
		<h4><?php esc_html_e('Donation Goal Settings', 'wc-invoice-payment'); ?></h4>
		...
		<p class="show_if_donation">
			<label for="_donation_enable_goal">
				...
				<?php esc_html_e('Enable donation goal', 'wc-invoice-payment'); ?>
				<i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e('Enable a donation goal for this product. When reached, donations will no longer be accepted.', 'wc-invoice-payment'); ?>"></i>
			</label>
		</p>
		...
		<p class="show_if_donation_goal">
			<label for="_donation_show_progress">
				...
				<?php esc_html_e('Show progress bar', 'wc-invoice-payment'); ?>
				<i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e('Display the donation progress bar on the product page.', 'wc-invoice-payment'); ?>"></i>
			</label>
		</p>
	</div>
	...
	<div class="donation-deadline-section dokan-section">
		<h4><?php esc_html_e('Donation Deadline Settings', 'wc-invoice-payment'); ?></h4>
		...
		<p class="show_if_donation">
			<label for="_donation_enable_deadline">
				...
				<?php esc_html_e('Enable donation deadline', 'wc-invoice-payment'); ?>
				<i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e('Set a deadline for donations. After this date, no more donations will be accepted.', 'wc-invoice-payment'); ?>"></i>
			</label>
		</p>
		...
		<p class="show_if_donation_deadline">
			<label for="_donation_show_countdown">
				...
				<?php esc_html_e('Show countdown timer', 'wc-invoice-payment'); ?>
				<i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e('Display a countdown timer on the product page showing time remaining until deadline.', 'wc-invoice-payment'); ?>"></i>
			</label>
		</p>
		
		<p class="show_if_donation_deadline">
			<label for="_donation_deadline_message"><?php esc_html_e('Deadline expired message', 'wc-invoice-payment'); ?> <i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e('Message to display when the donation deadline has passed.', 'wc-invoice-payment'); ?>"></i></label>
